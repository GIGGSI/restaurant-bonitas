import food1 from '../images/food1.jpg'
import GreekSalat from '../images/fullMenu/greekSalat.jpg'
import salat1 from '../images/gal-1.jpg'
import salat2 from '../images/salat2.jpg'
import cesar from '../images/fullMenu/cesar.jpg'
import nisoas from '../images/fullMenu/nisoas.jpg'

import tarama from '../images/fullMenu/tarama.jpg'
import humus from '../images/fullMenu/humus.jpg'
import guakamole from '../images/fullMenu/guakamole.jpg'
import artishok from '../images/fullMenu/artishok.jpg'
import tikvichki from '../images/fullMenu/tikvichki.jpg'
import halumi from '../images/fullMenu/halumi.jpg'
import falafel from '../images/fullMenu/falafel.jpg'
import skaridi from '../images/fullMenu/skaridi.jpg'
import kalmari from '../images/fullMenu/kalmari.jpg'
import dondu from '../images/fullMenu/dondu.jpg'
import pile from '../images/fullMenu/pile.jpg'

import margarita from '../images/fullMenu/margarita.jpg'
import kaprichoza from '../images/fullMenu/kaprichoza.jpg'
import pizzaPile from '../images/fullMenu/pizzaPile.jpg'
import peperoni from '../images/gal-3.jpg'
import krudo from '../images/fullMenu/krudo.jpg'
import durpano from '../images/fullMenu/Durpano.jpg'
import tortila from '../images/fullMenu/tortila.jpg'
import burger from '../images/burger.jpg'

import boloneze from '../images/fullMenu/boloneze.jpg'
import salmon from '../images/fullMenu/salmon.jpg'
import carbonara from '../images/fullMenu/carbonara.jpg'
import rissoto1 from '../images/fullMenu/rissoto1.jpg'
import chicken from '../images/fullMenu/chicken.jpg'
import beef from '../images/fullMenu/beef.jpg'

import djolan from '../images/fullMenu/djolan.jpg'
import turkey from '../images/fullMenu/turkey.jpg'
import beefRibs from '../images/fullMenu/beefRibs.jpg'
import beefBon from '../images/fullMenu/beefBon.jpg'
import shnicel from '../images/shnicel.jpg'
import salmonWithSoanich from '../images/fullMenu/salmonWithSpanich.jpg'
import chickenFillet from '../images/fullMenu/chickenFillet.jpg'
import beefMeatbolls from '../images/fullMenu/beefMeatbolls.jpg'
import pork from '../images/fullMenu/pork.jpg'
import ribay from '../images/fullMenu/ribay.jpg'
import seaBass from '../images/fullMenu/seaBass.jpg'

import tiramisu from '../images/tiramisu.jpg'
import sufle from '../images/sufle.jpg'
import cheesseCake from '../images/fullMenu/cheeseeCake.jpg'
import maskarpone from '../images/fullMenu/maskarpone.jpg'

const FullMenu = [

    {
        id: 1,
        title: 'Гръцка салата',
        category: 'Салати',
        price: 10.99,
        img: GreekSalat,
        desc: `Традиционна гръцка салата по оригинална рецепта !   
 
        `,
    },
    {
        id: 2,
        title: 'Домати, печени зеленчуци и сирене',
        category: 'Салати',
        price: 9.99,
        img: salat2,
        desc: `Лесна, бърза и здравословна салата с патладжан и още куп любими зеленчуци
        `,
    },
    {
        id: 3,
        title: 'Салата с орехи, нар и козе сирене',
        category: 'Салати',
        price: 11.99,
        img: salat1,
        desc: `
        Невероятно вкусна салата с микс от зелени слати, орехи нар и козе сирене..
        `,
    },
    {
        id: 4,
        title: 'Салата Цесар с пиле',
        category: 'Салати',
        price: 11.99,
        img: cesar,
        desc: `
        Топли, хрупкави, италиански подправени крутони. Сочно пиле. Сирене вафли пармезан. Хрупкава маруля. Кремообразен, солен дресинг
        `,
    },
    {
        id: 5,
        title: 'Салата Нисоаз',
        category: 'Салати',
        price: 12.99,
        img: nisoas,
        desc: `
        Салата Нисоаз е Френска, богата на много вкусове и определено си заслужава вниманието.
        `,
    },
    {
        id: 6,
        title: 'Розова тарама хайвер',
        category: 'Предястия',
        price: 5.99,
        img: tarama,
        desc: `
        Тарама хайверът се е наложил в днешното меню като чудесно предястие. Особено предпочитан е в комбинация с препечени многозърнести и пълнозърнести хлебчета.
        `,
    },
    {
        id: 7,
        title: 'Хумус',
        category: 'Предястия',
        price: 6.99,
        img: humus,
        desc: `
        Една от най-популярните и вкусни арабски разядки!        `,
    },
    {
        id: 8,
        title: 'Артишок',
        category: 'Предястия',
        price: 6.99,
        img: artishok,
        desc: `
        Артишок с маслини и чери домати!      `,
    },
    {
        id: 9,
        title: 'Гуакамоле с козе сирене',
        category: 'Предястия',
        price: 8.99,
        img: guakamole,
        desc: `
        Здравословно, вкусно и при това диетично – перфектното описание за мексиканското ястие. `,
    },
    {
        id: 10,
        title: 'Тиквички по гръцки',
        category: 'Предястия',
        price: 7.99,
        img: tikvichki,
        desc: `
        Хрупкавите тиквички по гръцки са любимо зеленчуково изкушение, особено през лятото и независимо дали сте близо до гръцкия морски бряг.          `,
    },
    {
        id: 11,
        title: 'Гриловано Халуми',
        category: 'Предястия',
        price: 11.99,
        img: halumi,
        desc: `
        Гриловано Халуми със сладко от горски плодове.
`    },
    {
        id: 12,
        title: 'Спаначени фалафели',
        category: 'Предястия',
        price: 11.99,
        img: falafel,
        desc: `
          Спаначени фалафели с таханов сос
       `
    },
    {
        id: 13,
        title: 'Скариди темпура със срирача сос',
        category: 'Предястия',
        price: 15.99,
        img: skaridi,
        desc: `
        Скариди темпура със срирача сос
       `
    },
    {
        id: 14,
        title: 'Калмари със сос',
        category: 'Предястия',
        price: 15.99,
        img: kalmari,
        desc: `
        Калмари хрупкави или на плоча.
       `
    },
    {
        id: 15,
        title: 'Фондю',
        category: 'Предястия',
        price: 15.99,
        img: dondu,
        desc: `
         Фондю от сирена и спанак с брускети.
       `
    },
    {
        id: 16,
        title: 'Пилешки филенца',
        category: 'Предястия',
        price: 15.99,
        img: pile,
        desc: `
        Хрупкави пилешки филенца с млечен сос.
       `
    },
    {
        id: 17,
        title: 'Маргарита',
        category: 'Пици и бургери',
        price: 9.99,
        img: margarita,
        desc: `
        Класическата неаполитанска пица Маргарита е една от пиците, които всеки обича.        `
    },
    {
        id: 18,
        title: 'Пица Капричоза',
        category: 'Пици и бургери',
        price: 11.99,
        img: kaprichoza,
        desc: `
        Пица Капричоза с шунка гъби, артишок и моцарела.
       `
    },
    {
        id: 19,
        title: 'Пица с Пиле',
        category: 'Пици и бургери',
        price: 11.99,
        img: pizzaPile,
        desc: `
        Пица с пиле, топено сирене, кисели краставички, царевица, моцарела.    
        `
    },
    {
        id: 20,
        title: 'Пица с Пеперони',
        category: 'Пици и бургери',
        price: 11.99,
        img: peperoni,
        desc: `
       Пица с Пеперони, чушки и моцарела /пикантна/ .
        `
    },
    {
        id: 21,
        title: 'Пица с Прошуто крудо',
        category: 'Пици и бургери',
        price: 11.99,
        img: krudo,
        desc: `
       Пица с Прошуто крудо, рукола, маскарпоне и моцарела .
        `
    },
    {
        id: 22,
        title: 'Бургер с Дърпано свинско месо',
        category: 'Пици и бургери',
        price: 11.99,
        img: durpano,
        desc: `
       Бургер с Дърпано свинско месо поднесено с пържени картофки.
        `
    },
    {
        id: 23,
        title: 'Телешки бургер',
        category: 'Пици и бургери',
        price: 11.99,
        img: burger,
        desc: `
       Телешки бургер с пържени картофи и BBQ сос.
        `
    },
    {
        id: 24,
        title: 'Тортила с Телешко месо',
        category: 'Пици и бургери',
        price: 11.99,
        img: tortila,
        desc: `
      Мексиканска тортила с телешко месо и млечен сос.
        `
    },
    {
        id: 25,
        title: 'Талиателе Болонезе',
        category: 'Паста и Ризото',
        price: 9.99,
        img: boloneze,
        desc: `
      Класически талиателе със сос Болонезе.
        `
    },
    {
        id: 26,
        title: 'Талиателе със сьомга, броколи и сметана ',
        category: 'Паста и Ризото',
        price: 14.90,
        img: salmon,
        desc: `
        Сочните парченца пушена сьомга, ореховите броколи и нежните макарони се сгъват през сметанов сос.        `
    },
    {
        id: 27,
        title: 'Талиателе Карбонара',
        category: 'Паста и Ризото',
        price: 10.90,
        img: carbonara,
        desc: `
        Богат, кремообразен, снизходителен. Пастата карбонара е най-добрата комфортна храна.     `
    },
    {
        id: 28,
        title: 'Ризото със зеленчуци, маскарпоне и пармезан',
        category: 'Паста и Ризото',
        price: 10.90,
        img: rissoto1,
        desc: `
        Ризото със зеленчуци, маскарпоне и пармезан     `
    },
    {
        id: 29,
        title: 'Ризото с пиле, маскарпоне и пармезан',
        category: 'Паста и Ризото',
        price: 11.90,
        img: chicken,
        desc:
            `
        Наистина кремообразно, богато и вкусно ризото от пиле и гъби.
    `
    },
    {
        id: 30,
        title: 'Ризото с телешко бонфиле',
        category: 'Паста и Ризото',
        price: 16.90,
        img: beef,
        desc:
            `
        Това богато телешко ризото, подправено с мащерка, гъби и аспержи, е идеалното основно за следващата ви вечеря.
    `
    },
    {
        id: 31,
        title: 'Бавно приготвен джолан',
        category: 'Основни',
        price: 18.90,
        img: djolan,
        desc:
            `
       Бавно приготвен джолан с мачкан картоф и билково масло.
    `
    },
    {
        id: 32,
        title: 'Пуешки стек',
        category: 'Основни',
        price: 18.90,
        img: turkey,
        desc:
            `
            Запържена пуешка гърда със сладко картофено пюре и празничен сос
    `
    },
    {
        id: 33,
        title: 'Глазирани телешки ребра',
        category: 'Основни',
        price: 18.90,
        img: beefRibs,
        desc:
            `Глазирани телешки ребра с пържени картофи `
    },
    {
        id: 34,
        title: 'Телешки шницел по виенски',
        category: 'Основни',
        price: 18.99,
        img: shnicel,
        desc: `Традиционен телешки шницел по виенски, поднесен с пържени картофи`,
    },
    {
        id: 35,
        title: 'Телешкo бон филе',
        category: 'Основни',
        price: 18.99,
        img: beefBon,
        desc: `Телешко бон филе с пепър сос и пюре от грах`,
    },
    {
        id: 36,
        title: 'Филе от сьомга',
        category: 'Основни',
        price: 18.99,
        img: salmonWithSoanich,
        desc: `Филе от сьомга с хрупкава коричка и спанак соте, киноа и пармезан`,
    },
    {
        id: 37,
        title: 'Мариновано пилешко филе',
        category: 'Основни',
        price: 18.99,
        img: chickenFillet,
        desc: `Мариновано пилешко филе с гъбен сос и мачкан картоф.`,
    },
    {
        id: 38,
        title: 'Кюфтета от телешко месо',
        category: 'Основни',
        price: 18.99,
        img: beefMeatbolls,
        desc: `Кюфтета от телешко месо с пържени картофи и BBQ сос.`,
    },
    {
        id: 39,
        title: 'Свински Томахоук с картофи',
        category: 'Основни',
        price: 22.99,
        img: food1,
        desc: `
        Приотвен на скара свински томахоук, поднесен с карофи.
        `,
    },
    {
        id: 40,
        title: 'Свинскa Вратна пържола',
        category: 'Основни',
        price: 22.99,
        img: pork,
        desc: `
     Свинска вратна пържола с пепър сос и пюре от грах.
        `,
    },
    {
        id: 41,
        title: 'Рибай стек',
        category: 'Основни',
        price: 22.99,
        img: ribay,
        desc: `
     Невроятен Рибай стек, сочен, перфектно приготвен.
        `,
    },
    {
        id: 42,
        title: 'Филе от лаврак',
        category: 'Основни',
        price: 12.99,
        img: seaBass,
        desc: `
    Филе от лаврак със спанак соте, киноа и пармезан.
        `,
    },

    {
        id: 43,
        title: 'Тирамису',
        category: 'Десерти',
        price: 3.99,
        img: tiramisu,
        desc: `
        Истинско изкушение и истинско прегрешение. Но е пролет и можем да си позволим да се поглезим малко.        `,
    },
    {
        id: 44,
        title: 'Шоколадово суфле',
        category: 'Десерти',
        price: 3.99,
        img: sufle,
        desc: `
        Любителите на сладкото ще се влюбят в това наистина вълнуващо сетивата гурме приключение.
        `,
    },
    {
        id: 45,
        title: 'Сладоледен чийзкейк',
        category: 'Десерти',
        price: 3.99,
        img: cheesseCake,
        desc: `
        Любителите на сладкото ще се влюбят в това наистина вълнуващо сетивата гурме приключение.
        `,
    },
    {
        id: 46,
        title: 'Крем маскарпоне',
        category: 'Десерти',
        price: 3.99,
        img: maskarpone,
        desc: `
       Крем маскарпоне с горски плодове.
        `,
    },


];
export default FullMenu;
